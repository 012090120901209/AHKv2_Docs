...
Returns the column number in an Edit control where the caret (text
...
This function returns the column number in an Edit control where the
caret (text insertion point) resides. The first column is 1. If there is
text selected in the control, the return value is the column number
